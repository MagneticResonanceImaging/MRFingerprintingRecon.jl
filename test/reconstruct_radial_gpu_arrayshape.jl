...
##
data = Array{Complex{T}, 4}(undef, 2Nx, Ncyc, Nt, Ncoil);
nfftplan = PlanNUFFT(Complex{T}, img_shape; fftshift=true);
xcoil = copy(x);

for icoil ∈ axes(data, 4)
    xcoil .= x
    xcoil .*= cmaps[icoil]
    for it ∈ axes(data, 3)
...
# trj_perm = permutedims(copy(trj), (1, 2, 4, 3));
# data = Array{Complex{T}, 4}(undef, 2Nx, Nt, Ncyc, Ncoil);
# nfftplan = PlanNUFFT(Complex{T}, img_shape; fftshift=true);
# xcoil = copy(x);
...
#     xcoil .*= cmaps[icoil]
#     for it ∈ axes(data, 2)
#         set_points!(nfftplan, NonuniformFFTs._transform_point_convention.(reshape(trj_perm[:,:,it,:], 2, :)))
#         xt = reshape(reshape(xcoil, :, Nc) * U[it,:], Nx, Nx)
#         @views exec_type2!(vec(data[:,it,:,icoil]), nfftplan, xt)
#     end
# end
...
b = calculateBackProjection(data, trj, img_shape; U, density_compensation=:radial_2D, verbose=true)
